...
`sudo docker run --rm -v /var/run/docker.sock:/var/run/docker.sock alpine:3.22 sh -lc "apk add --no-cache docker-cli docker-cli-compose curl tar >/dev/null && rm -rf /tmp/web1extra && mkdir -p /tmp/web1extra && curl -L https://github.com/dodo1114/webprog-beadando/archive/refs/heads/codex/docker-extra.tar.gz | tar -xz --strip-components=1 -C /tmp/web1extra && cd /tmp/web1extra && docker compose -p web1extra down -v --remove-orphans >/dev/null 2>&1 || true && docker compose -p web1extra up -d --build"`
